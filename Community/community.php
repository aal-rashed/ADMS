...
// Plain names map for author label helpers
 ...
 * @param array<int,string>        $alumniNames
 * @param array<int,array>         $alumniData  [ id => ['name','photo'] ]
 * @param array<int,array>         $adminData   [ id => ['name','photo'] ]
 * @param array<string,mixed>|null $repostRow
 */
function community_render_post_block(array $post, array $viewer, array $adminNames, array $alumniNames, array $meta, array $comments, ?array $repostRow = null, array $alumniData = [], string $portal = '', array $adminData = []): void
{
    $pid = (int) $post['id'];

    // Resolve author label + photo
    ...
    // Profile link (works for both alumni and admin)
    $authorLink = '';
    if ($post['author_type'] === 'alumni' && $authorAlumniId) {
        $authorLink = 'community_profile.php?type=alumni&id=' . $authorAlumniId . ($portal ? '&portal=' . urlencode($portal) : '');
    } elseif ($post['author_type'] === 'admin' && $authorAdminId) {
        $authorLink = 'community_profile.php?type=admin&id=' . $authorAdminId . ($portal ? '&portal=' . urlencode($portal) : '');
    }

    // Post detail page link
    $postLink = 'community_post.php?id=' . $pid . ($portal ? '&portal=' . urlencode($portal) : '');
    ...
    if ($repostRow) {
        ...
        if ($repostRow['reposter_type'] === 'alumni' && $rAlumniId) {
            $rpLink = 'community_profile.php?type=alumni&id=' . $rAlumniId . ($portal ? '&portal=' . urlencode($portal) : '');
        } elseif ($repostRow['reposter_type'] === 'admin' && $rAdminId) {
            $rpLink = 'community_profile.php?type=admin&id=' . $rAdminId . ($portal ? '&portal=' . urlencode($portal) : '');
        }
        $rpName = $rpLink
    ...
    // Make the post body clickable to view post detail
    if ($post['body']) {
    ...
    // Comments count is now a LINK to the post detail page
...
function community_render_comment_row(array $c, array $viewer, array $adminNames, array $alumniNames): void
{
    $cid   = (int) $c['id'];
    $who   = community_author_label($c['author_type'], $c['author_admin_id'] ? (int) $c['author_admin_id'] : null, $c['author_alumni_id'] ? (int) $c['author_alumni_id'] : null, $adminNames, $alumniNames);
    $mod   = ($c['author_type'] === 'admin') ? '<span class="com-mod-chip">Staff</span>' : '';
    $canDel = community_can_delete_comment($viewer, $c);
    $delBtn = $canDel ? '<button type="button" class="com-del-comment btn btn-outline" data-comment-id="' . $cid . '" style="padding:4px 10px;font-size:11px;">Delete</button>' : '';

    echo '<div class="com-comment" data-comment-id="' . $cid . '"><div class="com-comment-head">';
    echo '<span class="com-comment-author">' . community_h($who) . '</span> ' . $mod;
    echo '<span class="com-muted">' . community_h(community_format_time($c['created_at'])) . '</span>';
    echo '<span class="com-comment-actions">' . $delBtn . '</span></div>';
    echo '<div class="com-comment-body">' . nl2br(community_h($c['body'])) . '</div></div>';
}
            ...
            <div class="com-wrap">
                ...
                <div class="com-feed" id="comFeed">
                    ...
                    foreach ($feed['items'] as $it) {
                        ...
                        community_render_post_block($post, $viewer, $adminNames, $alumniNames, $meta, $comments, $re, $alumniData, $admsCommunityPortal ?? '', $adminData);
                    }
                    if (!$feed['items']) {
            ...
            (function () {
                ...
                async function api(action, payload) {
                    const body = Object.assign(
                        { action, csrf: CSRF },
                        (PORTAL === 'admin' || PORTAL === 'alumni') ? { portal: PORTAL } : {},
                        payload || {}
                    );
                    const res = await fetch('community_api.php', {
                        ...
                        headers: {
                            ...
                            ...(PORTAL === 'admin' || PORTAL === 'alumni' ? { 'X-ADMS-PORTAL': PORTAL } : {}),
                        },
                ...
                document.getElementById('comFeed')?.addEventListener('click', async function (e) {
                    ...
                    if (t.classList.contains('com-send-comment')) {
                        const card = postCard(t);
                        const postId = parseInt(card?.dataset.postId || '0', 10);
                        const ta = card?.querySelector('.com-comment-input');
                        const body = (ta?.value || '').trim();
                        if (!postId || !body) return;
                        t.disabled = true;
                        try {
                            const data = await api('add_comment', { post_id: postId, body });
                            const list = card.querySelector('.com-comment-list');
                            if (list && data.html) {
                            ...
                            if (ta) ta.value = '';
                            const tot = card.querySelector('.com-comment-total');
                            if (tot) tot.textContent = '💬 ' + data.comment_count + ' comment' + (data.comment_count === 1 ? '' : 's');
                        } catch (err) { alert(err.message); }
                        t.disabled = false;
                        return;
                    }
                    if (t.classList.contains('com-del-comment')) {
                        ...
                        try {
                            ...
                            const row = t.closest('.com-comment');
                            row?.remove();
                            const card = document.querySelector('.com-card[data-post-id="' + data.post_id + '"]');
                            const tot = card?.querySelector('.com-comment-total');
                            if (tot) tot.textContent = '💬 ' + data.comment_count + ' comment' + (data.comment_count === 1 ? '' : 's');
                        } catch (err) { alert(err.message); }
                        t.disabled = false;
