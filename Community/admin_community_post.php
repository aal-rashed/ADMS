<?php
/**
 * Admin Community Post Detail Page
 * Shows a single post with all its comments below it.
 * Hardcoded to admin portal.
 */
session_start();
require __DIR__ . '/config.php';
require __DIR__ . '/community_lib.php';

// Guard: only admins (admin portal uses default PHP session)
...
if ($ready && !$post) {
    ...
    echo '<!DOCTYPE html><html><body style="font-family:sans-serif;padding:40px;"><h2>Post not found</h2><a href="admin_community.php">← Back to Community</a></body></html>';
    exit;
}
...
if ($ready && $post) {
    ...
    // Liked / reposted by admin viewer
...
/** Helper: profile link for admin portal */
function admin_post_profile_link(string $type, ?int $adminId, ?int $alumniId): string
{
    if ($type === 'alumni' && $alumniId) {
        return 'admin_community_profile.php?type=alumni&id=' . $alumniId;
    }
    if ($type === 'admin' && $adminId) {
        return 'admin_community_profile.php?type=admin&id=' . $adminId;
    }
    return '';
}
?>
...
<head>
    ...
    <title>Post – Community – ADMS Admin</title>
...
<div class="layout">
    ...
    <main class="main-content animate-fadeIn">
        ...
        <a class="back-link" href="admin_community.php">← Back to Community</a>
        ...
        <?php else:
            ...
            $profileLink = admin_post_profile_link($post['author_type'], $authorAdminId, $authorAlumniId);
            ...
            <section class="post-comments-section">
                ...
                <?php else:
                    foreach ($comments as $c):
                        ...
                        $cLink   = admin_post_profile_link($c['author_type'], $cAdId, $cAlId);
        ...
        (function () {
            ...
            card.querySelector('.com-del-post')?.addEventListener('click', async function () {
                ...
                try {
                    ...
                    window.location.href = 'admin_community.php';
                } catch (err) { alert(err.message); }
            });
