 ...
 * Reached by clicking a post in the community feed.
 */
...
if ($ready && !$post) {
    ...
    echo '<!DOCTYPE html><html><body style="font-family:sans-serif;padding:40px;"><h2>Post not found</h2><a href="community.php' . htmlspecialchars($admsCommunityPortal ? '?portal=' . $admsCommunityPortal : '') . '">← Back to Community</a></body></html>';
    exit;
}
...
if ($ready && $post) {
    ...
    // Liked / reposted by viewer
    if ($viewer['type'] === 'admin' && $viewer['admin_id']) {
        $st = $pdo->prepare("SELECT id FROM community_likes WHERE post_id = ? AND liker_type='admin' AND liker_admin_id = ?");
        $st->execute([$postId, (int) $viewer['admin_id']]);
        if ($st->fetch()) $meta['liked'][$postId] = true;

        $st = $pdo->prepare("SELECT id FROM community_reposts WHERE original_post_id = ? AND reposter_type='admin' AND reposter_admin_id = ?");
        $st->execute([$postId, (int) $viewer['admin_id']]);
...
// Resolve names + photos for author and all commenters
$adminIds = [];
$alumniIds = [];
if ($post) {
...
$portalQS = $admsCommunityPortal ? '&portal=' . urlencode($admsCommunityPortal) : '';

/* Helpers used in this view */
function post_profile_link(string $type, ?int $adminId, ?int $alumniId, string $portal): string
{
    if ($type === 'alumni' && $alumniId) {
        return 'community_profile.php?type=alumni&id=' . $alumniId . ($portal ? '&portal=' . urlencode($portal) : '');
    }
    if ($type === 'admin' && $adminId) {
        return 'community_profile.php?type=admin&id=' . $adminId . ($portal ? '&portal=' . urlencode($portal) : '');
    }
    return '';
}
?>
...
<head>
    ...
    <title>Post – Community – ADMS</title>
...
<div class="layout">
    ...
    <main class="main-content animate-fadeIn">
        ...
        <?php else:
            ...
            $profileLink = post_profile_link($post['author_type'], $authorAdminId, $authorAlumniId, $admsCommunityPortal ?? '');
            ...
            <section class="post-comments-section">
                ...
                <?php else:
                    foreach ($comments as $c):
                        ...
                        $cLink   = post_profile_link($c['author_type'], $cAdId, $cAlId, $admsCommunityPortal ?? '');
        ...
        (function () {
            ...
            card.querySelector('.com-del-post')?.addEventListener('click', async function () {
                ...
                try {
                    ...
                    window.location.href = 'community.php' + (PORTAL ? '?portal=' + PORTAL : '');
                } catch (err) { alert(err.message); }
            });
            ...
            document.getElementById('commentList')?.addEventListener('click', async function (e) {
                ...
                try {
                    ...
                    updateCommentTotal(data.comment_count);
                    /* Re-add placeholder if list becomes empty */
                    const list = document.getElementById('commentList');
