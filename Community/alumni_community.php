<?php
/**
 * Alumni Community Page — alumni-only portal.
 * Hardcoded to portal=alumni. Reached from alumni_sidebar.php.
 */
...
// Guard: only alumni may use this page
...
/**
 * Render a single post block. Uses alumni-specific page filenames for links.
 */
function alumni_community_render_post_block(array $post, array $viewer, array $adminNames, array $alumniNames, array $meta, array $comments, ?array $repostRow = null, array $alumniData = [], array $adminData = []): void
{
    $pid = (int) $post['id'];
    ...
    // Profile link → alumni profile page
    $authorLink = '';
    if ($post['author_type'] === 'alumni' && $authorAlumniId) {
        $authorLink = 'alumni_community_profile.php?type=alumni&id=' . $authorAlumniId;
    } elseif ($post['author_type'] === 'admin' && $authorAdminId) {
        $authorLink = 'alumni_community_profile.php?type=admin&id=' . $authorAdminId;
    }

    // Post detail page link → alumni post page
    $postLink = 'alumni_community_post.php?id=' . $pid;
    ...
    if ($repostRow) {
        ...
        if ($repostRow['reposter_type'] === 'alumni' && $rAlumniId) {
            $rpLink = 'alumni_community_profile.php?type=alumni&id=' . $rAlumniId;
        } elseif ($repostRow['reposter_type'] === 'admin' && $rAdminId) {
            $rpLink = 'alumni_community_profile.php?type=admin&id=' . $rAdminId;
        }
        $rpName = $rpLink
...
<head>
    ...
    <title>Community – Alumni Portal</title>
            ...
            <div class="com-wrap">
                <div class="com-composer card" style="padding:22px;">
                    ...
                    <form id="comCreateForm" enctype="multipart/form-data">
                        ...
                        <input type="hidden" name="portal" value="alumni">
                        <div class="field">
                ...
                <div class="com-feed" id="comFeed">
                    ...
                    foreach ($feed['items'] as $it) {
                        ...
                        alumni_community_render_post_block($post, $viewer, $adminNames, $alumniNames, $meta, $comments, $re, $alumniData, $adminData);
                    }
                    if (!$feed['items']) {
